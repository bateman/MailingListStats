...
#
# Authors:
#     GSyC/LibreSoft Group <libresoft-tools-devel@lists.morfeo-project.org>
#
...
class FileExtractor(object):
    ...
    def tarExtraction(self, filename):
        ...
        try:
            ...
            raise FileExtractorError("Opening tarfile %s" % filename)
        ...
        for tarinfo in tar:
            try:
                ...
                tar.close()
                raise FileExtractorError("Extracting tarfile %s" % filename)
    ...
    def zipExtraction(self, filename):
        ...
        try:
            ...
            raise FileExtractorError("Opening zipfile %s" % filename)
        ...
        for name in zipped.namelist():
            try:
                ...
                # If subdirectories do not exist, the creation of the
                # file will fail
                ...
                zipped.close()
                raise FileExtractorError('Reading %s zipfile' % filename)
            except IOError:
                zipped.close()
                raise FileExtractorError('Write error while extracting'
                                         ' %s zipfile' % filename)
    ...
    def extract(self, path_list):
        ...
        The algorithm is the next:
            ...
            Steps:
                ...
                - Generate a LIFO list that will contains the path to
                  files or directories containers.
                ...
                - Do until the list isn't be empty:
                  ...
                  2) Check if the path belongs to a file or a directory
                     ...
                     a) If the element is a directory:
                        a.1) Add the elements than contains the directory at
                             the end of the list

                     b) If the element is a file:
                        b.1) Check if the file is a supported container
                             (i.e tar, zip, deb, rpm, and so on)
                        b.2) If it's supported:
                             b.2.1) extract
                             b.2.2) for each extracted element, add its
                                    path to the end of the list

                  3) Goto loop's start

        @param path_list: list with paths of files or directories to
                          be extracted
        ...
        while len(container_list) > 0:
            ...
            if not os.path.exists(container):
                raise FileExtractorError("%s not found" % container)
